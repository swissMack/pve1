...
// Initialize Anthropic client (reads from ANTHROPIC_API_KEY env variable)
...
const llmRateLimitMap = new Map()
const LLM_RATE_LIMIT_WINDOW = 60000
const LLM_RATE_LIMIT_MAX = 20

function checkLlmRateLimit(userId) {
  const now = Date.now()
  const userLimit = llmRateLimitMap.get(userId) || { count: 0, resetTime: now + LLM_RATE_LIMIT_WINDOW }
  if (now > userLimit.resetTime) {
    userLimit.count = 1
    userLimit.resetTime = now + LLM_RATE_LIMIT_WINDOW
  } else {
    userLimit.count++
  }
  llmRateLimitMap.set(userId, userLimit)
  return userLimit.count <= LLM_RATE_LIMIT_MAX
}

// Build system prompt based on user role
async function buildLlmSystemPrompt(role) {
  let contextData = ""
  try {
    const devicesResult = await pool.query(`SELECT COUNT(*) as total, status FROM ${SCHEMA}devices GROUP BY status`)
    const deviceStats = devicesResult.rows.reduce((acc, r) => { acc[r.status] = parseInt(r.total); return acc }, {})
    const simsResult = await pool.query(`SELECT COUNT(*) as total, status FROM ${SCHEMA}sim_cards GROUP BY status`)
    const simStats = simsResult.rows.reduce((acc, r) => { acc[r.status] = parseInt(r.total); return acc }, {})
    contextData = `Current System Status: Devices: ${JSON.stringify(deviceStats)}, SIM Cards: ${JSON.stringify(simStats)}`
  } catch (e) {
    console.error("Error fetching context data:", e.message)
  }
  return `You are "Ask Bob", an AI assistant for the IoTo SIM Card Portal. You help users manage IoT devices, SIM cards, and connectivity. User Role: ${role}. ${contextData}. Guidelines: Be concise and helpful. For data queries, provide summaries. Dont expose sensitive data like full ICCIDs.`
}
...
app.post("/api/llm/chat", async (req, res) => {
  try {
    ...
    const systemPrompt = await buildLlmSystemPrompt(role)
    ...
    res.json({ success: true, message: responseText, actions: [] })
  } catch (error) {
...
app.post("/api/llm/chart", async (req, res) => { res.json({ success: true, config: null }) })
app.post("/api/llm/execute", async (req, res) => { res.json({ success: true, result: "Not implemented" }) })
app.get("/api/llm/pending-actions", async (req, res) => { res.json({ success: true, actions: [] }) })
