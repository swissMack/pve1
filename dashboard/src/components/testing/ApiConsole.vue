...
type TestType = 'sim-lookup'

const testOptions = [
  { label: 'SIM Lookup by ICCID', value: 'sim-lookup' as TestType }
]

const selectedTest = ref<TestType>('sim-lookup')
const iccidInput = ref('')
const loading = ref(false)
const result = ref<Sim | null>(null)
...
const runTest = async () => {
  ...
  try {
    ...
    error.value = {
      message: apiError.displayMessage || 'API request failed',
